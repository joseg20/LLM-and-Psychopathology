...
desired_order = ['Abstract - Semantic Distance',
                 'Abstract - Simplex Volume',
                 'Concrete - Semantic Distance',
                 'Concrete - Simplex Volume']
...
def process_and_normalize(df, label):
    ...
    grouped_stds = df_std.groupby([df['Induced Pathology'], df['Category']])[['Average Sequential Distance', 'Simplex Volume']].std().reset_index()
    
    grouped_stds = grouped_stds[grouped_stds['Category'] != 'Unknown']

    grouped_means = df.groupby(['Induced Pathology', 'Category'])[['Average Sequential Distance', 'Simplex Volume']].mean().reset_index()
    
    grouped_means = grouped_means[grouped_means['Category'] != 'Unknown']
    ...
    grouped_ordered = grouped_means.melt(id_vars=['Induced Pathology', 'Category'], value_vars=['Average Sequential Distance', 'Simplex Volume'])
    
    grouped_ordered['Metric_Category'] = grouped_ordered['Category'] + ' - ' + grouped_ordered['variable'].replace('Average Sequential Distance', 'Semantic Distance')
    grouped_ordered['Metric_Category'] = pd.Categorical(grouped_ordered['Metric_Category'], categories=desired_order, ordered=True)
    ...
    stds_ordered = grouped_stds.melt(id_vars=['Induced Pathology', 'Category'], value_vars=['Average Sequential Distance', 'Simplex Volume'])
...
for category in categories:
    ...
    gs = GridSpec(4, 1, figure=fig, height_ratios=[0.05, 1, 1, 1])
    
    fig.suptitle(f'Semantic Expansion ({category})', fontsize=24, fontweight='bold', y=0.985)
    
    for j, dataset in enumerate(datasets):
        ax_dataset = fig.add_subplot(gs[j+1, 0]) 
        
        data_filtered = data[(data['Dataset'] == dataset) & (data['Category'] == category)]
        stds_filtered = stds[(stds['Dataset'] == dataset) & (stds['Category'] == category)]
        ...
        if j == len(datasets) - 1:
            ...
            ax_dataset.set_xticklabels(['' for _ in range(len(pathologies_induced))])
    ...
    ax_legend.legend(handles, labels, loc='center', ncol=2, fontsize=16)
    ax_legend.axis('off')
    
    plt.tight_layout()
    plt.subplots_adjust(top=0.95)
    print(f'{ruta_guardado}{model}/{temp}/exp2_{category.lower()}_{model}_{temp}.png')
    plt.savefig(f'{ruta_guardado}{model}/{temp}/exp2_{category.lower()}_{model}_{temp}.png')